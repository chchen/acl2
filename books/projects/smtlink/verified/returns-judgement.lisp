...
(define filter-judges ((judge pseudo-termp)
                       ...
                       (acc pseudo-termp)
                       (seen pseudo-term-listp))
  :returns (mv (new-judge pseudo-termp)
               (new-seen pseudo-term-listp))
  :measure (acl2-count (pseudo-term-fix judge))
  :verify-guards nil
  (b* ((judge (pseudo-term-fix judge))
       (filter (pseudo-term-fix filter))
       (acc (pseudo-term-fix acc))
       (seen (pseudo-term-list-fix seen))
       ((if (and (type-predicate-p judge supertypes)
                 (path-test filter judge)
                 (not (member-equal judge seen))))
        (mv `(if ,judge ,acc 'nil) (cons judge seen)))
       ((if (type-predicate-p judge supertypes)) (mv acc seen))
       ((unless (is-conjunct? judge))
        (mv `(if ,judge ,acc 'nil) (cons judge seen)))
       ((if (equal judge ''t)) (mv acc seen))
       ((list & judge-left judge-right &) judge)
       ((mv left-acc left-seen)
        (filter-judges judge-left filter supertypes acc seen)))
    (filter-judges judge-right filter supertypes left-acc left-seen)))
...
(defthm correctness-of-filter-judges
    (implies (and (ev-smtcp-meta-extract-global-facts)
                  ...
                  (ev-smtcp judge a)
                  (ev-smtcp acc a))
             (ev-smtcp (mv-nth 0 (filter-judges judge filter supertype acc seen)) a))
...
(define generate-judge-alist-one ((judges pseudo-termp)
                                  ...
                                  (supertypes type-to-types-alist-p)
                                  (acc pseudo-termp)
                                  (type-pred pseudo-termp))
  :returns (mv (term-judge pseudo-termp)
  ...
  (b* ((judges (pseudo-term-fix judges))
       ...
       (acc (pseudo-term-fix acc))
       (type-pred (pseudo-term-fix type-pred))
       ((if (and (not (type-predicate-p judges supertypes))
                 (judgement-of-term judges term supertypes)))
        (mv `(if ,judges ,acc 'nil) type-pred))
       ((if (and (type-predicate-p judges supertypes)
                 (judgement-of-term judges term supertypes)
                 (not (equal type-pred ''t))))
        (mv acc type-pred))
       ((if (and (type-predicate-p judges supertypes)
                 (judgement-of-term judges term supertypes)
                 (equal type-pred ''t)))
        (mv `(if ,judges ,acc 'nil) judges))
       ((unless (is-conjunct? judges)) (mv acc type-pred))
       ((if (equal judges ''t)) (mv acc type-pred))
       ((list & judge-left judge-right &) judges)
       ((mv new-acc pred-left)
        (generate-judge-alist-one judge-left term supertypes acc type-pred)))
    (generate-judge-alist-one judge-right term supertypes new-acc pred-left)))
...
(defthm correctness-of-generate-judge-alist-one
  (implies (and (ev-smtcp-meta-extract-global-facts)
                ...
                (ev-smtcp judges a)
                (ev-smtcp acc a))
           (ev-smtcp (mv-nth 0 (generate-judge-alist-one
                                judges term supertypes acc type-pred))
                     a))
  :hints (("Goal"
...
(define generate-judge-alist ((judges pseudo-termp)
  ...
  (b* ((actuals (pseudo-term-list-fix actuals))
       ...
       ((mv judge-hd &)
        (generate-judge-alist-one judges ac-hd supertypes ''t ''t)))
    (acons ac-hd judge-hd (generate-judge-alist judges ac-tl supertypes))))
...
(define choose-returns ((returns-judge pseudo-termp)
  ...
  (b* ((returns-judge (pseudo-term-fix returns-judge))
       ...
       ((mv judges &) (filter-judges actuals-judge hypo supertypes ''t nil))
       (judge-alst (generate-judge-alist judges actuals supertypes)))
